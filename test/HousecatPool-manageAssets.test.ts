...
describe('HousecatPool: manageAssets', () => {
  it('only manager allowed to call', async () => {
    ...
    const manage = pool.connect(signer).manageAssets(adapters.uniswapV2.address, data)
  ...
  describe('UniswapV2Adapter', () => {
    it('manager should be able to trade tokens on behalf of the pool', async () => {
      const [signer, treasury, manager] = await ethers.getSigners()
      const { pool, adapters, amm, weth, tokens } = await mockHousecatAndPool(signer, treasury, manager)

      // deposit ETH so that the pool has WETH
      await pool.connect(manager).deposit({ value: parseEther('1') })

      // trade all weth to token0
      const data = adapters.uniswapV2.interface.encodeFunctionData('trade', [
        amm.address,
        [weth.token.address, tokens[0].token.address],
        await weth.token.balanceOf(pool.address),
        1,
      ])
      await pool.connect(manager).manageAssets(adapters.uniswapV2.address, data)
    ...
    it('should fail to trade on a router which is not whitelisted', async () => {
      const [signer, treasury, manager] = await ethers.getSigners()
      const { pool, adapters, weth, tokens } = await mockHousecatAndPool(signer, treasury, manager)
      const data = adapters.uniswapV2.interface.encodeFunctionData('trade', [
        routers.sushiswap,
        [weth.token.address, tokens[0].token.address],
        await weth.token.balanceOf(pool.address),
      ...
      const manage = pool.connect(manager).manageAssets(adapters.uniswapV2.address, data)
    ...
    it('should succeed to sell tokens that are not whitelisted', async () => {
      const [signer, treasury, manager] = await ethers.getSigners()
      const { management, pool, adapters, weth, tokens, amm } = await mockHousecatAndPool(signer, treasury, manager)
