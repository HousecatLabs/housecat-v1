...
describe('HousecatPool: manageAssets', () => {
  it('only manager allowed to call', async () => {
    ...
    const manage = pool.connect(signer).manageAssets(managerAdapters.uniswapV2.address, data)
  ...
  describe('UniswapV2Adapter', () => {
    it('manager should be able to trade tokens on behalf of the pool', async () => {
      ...
      await pool.connect(manager).manageAssets(managerAdapters.uniswapV2.address, data)
    ...
    it('should fail to trade on a router which is not whitelisted', async () => {
      ...
      const manage = pool.connect(manager).manageAssets(managerAdapters.uniswapV2.address, data)
